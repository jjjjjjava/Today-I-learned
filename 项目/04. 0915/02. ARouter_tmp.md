...
- 在 ARouter 的 init() 方法中会调用到 LogisticsCenter 的 init() 方法，LogisticsCenter 的 init() 方法会根据 registerByPlugin 的值判断路由表是否已经在编译时插入，如果不是的话，就通过运行时反射注册路由表的信息。（这里确定是反射吗？其实还有一种方式，从sharedPerference中加载）
...
- `编译时插入`就是由 `RegisterTransform` 从 `Jar` 文件中读取路由表的信息。 RegisterTransform 继承了 `Transform`  类，Transform API 是 Android Gradle 插件提供的一个编译期字节码处理接口，每个 Transform 实例就是一个 Gradle 任务，按顺序执行，能读取和处理 jar、aar 和 resource 等资源，并对他们进行扫描，修改，插桩。用户自定义的 Transform 会插在 Transform 队列的最前面（大致原理是对的，但是具体要细看）
