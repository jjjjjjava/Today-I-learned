...
解码的含义就是调用MediaCodec，其最终会借助硬件codec（调用芯片内置的视频解码器，如GPU/DSP）或软件codec（纯CPU计算，如FFmpeg、libstagefright）将H264解码为YUV流，将AAC解码后得到PCM数据。怎么理解：H264到YUV，怎么理解MediaCodec，怎么理解AAC到PCM？
...
好，我们知道解码借助MediaCodec完成，那么请问，MediaCodec如何实现解码的过程呢？

好，接着来看MediaCodec进行解码的数据流动

- 数据流如下图：我们可以看到有input客户端，input buff，output 客户端， output buff。
...
- MediaCodec的核心就是“缓冲区队列 + 状态机”机制，你和解码器通过input/output buffer交互数据，通过缓冲区队列，你和MediaCodec各自处理自己的部分，互不阻塞，提高效率。

那解码时MediaCodec的状态变化是什么样的呢？

- 状态变化如下：

  - <img src="../../_pic_/image-20250921160800345.png" alt="image-20250921160800345" style="zoom:80%;" />

- 我们可以看到有三种状态：停止，执行和释放。

- Stopped（停止态）

  - Uninitialized：MediaCodec对象刚被创建，还没配置参数，没被初始化。
...
-  Executing（执行态）
  ...
  - Flushed：准备状态，调用start()后，解码器已准备好，输入/输出缓冲区已清空，等待数据输入。
...
- 解码循环阶段，我们这里以同步模式为例
  ...
  - 渲染：将解码后的数据（如YUV帧）渲染到Surface、OpenGL或转成Bitmap显示（渲染又是很大的一块，在后面的OpenGl中我们再说）
...
好，我们已经知道解码的基础原理了。我们知道解码是要做什么，我们也知道解码的数据流，状态变化和解码的过程。接下来，请你来自己设计一个解码框架，借助MediaCodec实现解码功能，要求：扩展性好，简洁易用，能实现基础功能。

好，对于这个解码框架，首先我们将整个解码流程抽象为一个解码基类：BaseDecoder。

这个解码流程，他必须要一个解码的能力，所以我们要有解码能力的接口IDecoder，以及BaseDecoder实现这个IDecoder。

先看看IDecoder，他应该是什么样的？他应该可以控制解码的流程；查询解码状态；获取媒体信息。
...
我们的基础解码器BaseDecoder要实现这个接口IDecoder，这种关系说明这个类BaseDecoder自己就是解码器，所以用“继承接口”。下面还有一个`protected var mExtractor: IExtractor? = null` 思考一下，和自身继承有什么不同？

好，我们现在先来看看BaseDecoder的参数。
...
- 然后是解码相关成员
  ...
  - mInputBuffers: Array<ByteBuffer>? / mOutputBuffers: Array<ByteBuffer>?：解码器的输入/输出缓冲区数组。注意：这里是Buffers，表示有很多Buffer。
...
MediaCodec需要我们不断地喂数据给输入缓冲，这些数据就是音视频文件中的一帧，我们该怎么从音视频数据中获取一帧的数据呢？

我们这里还要有获取音视频数据源的能力，就像上面的解码器能力一样。我们要有一个成员变量实现IExtractor这个音视频数据读取器接口，解耦数据源和解码器，支持多种数据来源（如本地文件、网络流、内存等）

> 这里看IDecoder 和 IExtractor，分别是类实现接口和成员变量实现接口
>
> - ![image-20250923131953356](../../_pic_/image-20250923131953356.png)
>
> 用abstract class BaseDecoder: IDecoder，是说“BaseDecoder本身就是一种解码器”，它承诺实现IDecoder规定的所有功能,这是一种“自我能力”的声明：“我能做解码”。
>
> 用protected var mExtractor: IExtractor? = null，是说“BaseDecoder内部需要依赖一个实现了IExtractor接口的对象，来完成数据读取”,这是一种“协作能力”的声明：“我需要别人帮我做数据提取”.
>
> 他们的差异
>
...
- 具体如下：
  ...
  - 前置判断：
    ...
    - 停止状态则中断循环
  - 输入数据：如果还没到流结尾（EOS），调用pushBufferToDecoder()，从Extractor读取数据，送入解码器输入缓冲区，返回值标记是否已经到达流结尾。
...
- 然后是初始化数据提取器和视频源参数：
  ...
  - 通过判断mExtractor.getFormat能否获取MediaFormat等判断是否要return false，结束或继续后续的流程。
...
- 具体如下：
  - 获取Buffers的index，同理上面的。1000：最多等待1000ms（1秒），如果填-1则无限等待。应该是阻塞等待，利用epoll机制
  
  - 不过这里传入：mBufferInfo：用于接收输出帧的元数据，也就是说，每次拉取输出缓冲区时，mBufferInfo都被更新，我们可以用他来
