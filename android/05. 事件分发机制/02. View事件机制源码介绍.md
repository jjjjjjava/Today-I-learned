...
- ```
  ...
  - 如果分发的目标child不为空，那么会调用子元素的dispatchTouchEvent()。如果子元素的dispatchTouchEvent()方法返回true，那么mFirstTouchTarget就会被赋值，同时跳出for循环。
...
- Android事件分发是先传递到ViewGroup，再由ViewGroup传递到子View或当前ViewGroup的View父类。
...
- 1.onTouch（）是在 OnTouchListener 中，onClick（）在onTouchEvent（）中，onTouch的的执行优先于onClick
- 2.每当控件被点击时：
