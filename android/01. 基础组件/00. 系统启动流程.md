...
类似Uboot等，会从存储设备中加载Linux内核到内存中，然后跳转到入口地址处执行。
...
当内核启动完成时，其会启动init进程。init进程是一个由内核启动的**第一个用户级进程**，它的进程号是1，父进程id号是0。

init进程是所有用户空间的鼻祖, 它会解析系统配置文件，启动servicemanager(binder服务管家，其功能为查询和注册服务)等系统服务和守护进程,同时也会启动 Zygote进程(Java进程的鼻祖，负责启动应用进程的母进程). 

`Zygote` ，它负责启动Java虚拟机，同时其创建的第一个进程就是 `System Server`，`System Server` 负责管理和启动整个 Java Framework 层。创建完 `System Server` 之后，`Zygote` 就会完全进入受精卵的角色，等待进行无性繁殖，创建应用进程。所有的应用进程都是由 `Zygote` 进程 fork 而来的，称之为 Java 世界的女娲也不足为过。

### 1.5 应用程序启动

此后，当用户点击某个应用程序图标时，Android 的 **ActivityManagerService** 会通过 Zygote 进程启动一个新的应用进程来运行该应用。
...
Zygote 的启动是在 `on boot` 阶段完成的。`init.rc` 文件中定义了 ` class_start core`，而 Zygote 作为核心服务之一属于 `core` 类

### 2.2 启动Zygote

在on boot阶段，init进程会解析init.zygote64_32.rc ，创建app_main进程执行Zygote启动流程，这个进程最终在执行过程中会更名为Zygote.
...
其中zygote所对应的可执行文件是`/system/bin/app_process`

init进程执行到解析`init.zygote64_32.rc` 文件的位置，解析完服务定义后，经过一系统函数调用，最终通过 `fork()` 创建一个新的进程，然后让该进程进入到 `execve()`，调用 `/system/bin/app_process64`，进入app_main.cpp的main()函数，也就是 Zygote 的执行流程中。

整体流程如下图：

![zygote启动](./../_pic_/zygote启动.jpg)
...
AppRuntime类的声明和实现均在app_main.cpp中，它是从AndroidRuntime类派生出来的，所以上述start函数使用的是基类AndroidRuntime的start。

### AndroidRuntime.start

```C++
C++ 代码解读复制代码// frameworks/base/core/jni/AndroidRuntime.cpp
...
当我们在 Java 中使用 `native` 关键字声明一个本地方法时，JVM 并不知道这个方法的实现，它只是定义了一个接口。通过 JNI，我们告诉 JVM 这个 `native` 方法的实现是在某个本地（native）库中。为了将这个本地方法和 C/C++ 代码关联起来，需要遵循特定的绑定机制。
...
**提问：jclass**：`jclass` 是 JNI 中的一个类型，表示 Java 类在 JVM 中的本地引用。
...
- `forkSystemServer()` 方法用于创建 `system_server` 进程。创建成功则返回一个 `Runnable`，并通过`r.run()`在子进程中执行 `SystemServer.main()`。
...
虽然这一步会增加 Zygote 启动的时间，但它可以提升应用启动的效率，因为每个新启动的应用会继承 Zygote 的内存内容，包括已经加载好的类和资源。
...
- `handleSystemServerProcess` 首先创建了 `system_server` 进程的类加载器，并调用 `ZygoteInit.zygoteInit` 方法。
...
**创建系统上下文**：调用 `createSystemContext()` 方法，创建 `SystemContext`，它是系统服务运行时的上下文环境。
...
假设通过startActivity来启动一个新的Activity，而这个Activity附属于一个还未启动的进程，则会启动一个新的进程，调用startActivity的进程通过Binder调用到ActivityManagerService中的方法，然后经过层层调用，会调用到Process.start()方法，代码如下所示：
