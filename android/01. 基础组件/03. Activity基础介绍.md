...
如果有fragment时，`Activity` 成为一个容器，`Fragment` 担当 UI 和交互逻辑，而`Activity` 管理 `Fragment` 的生命周期
...
- 特殊场景：如果在Service或Application中启动一个Activity，其并没有所谓的任务栈，可以使用标记位Flag来解决。
